...
# Import CSVs
table_RANO = pd.read_csv("./lumiere/csvs/LUMIERE-ExpertRating_summary.csv", delimiter = ",",header = 0)
table_completeness = pd.read_csv("./lumiere/csvs/LUMIERE-datacompleteness.csv", delimiter = ",",header = 0)
...
for column in ["LessThan3Months","RANO","NonMeasurableLesions","RatingRationale","CT1", "T1", "T2","FLAIR","DeepBraTumIA","HD-GLIO-AUTO","DeepBraTumIA-CoLlAGe","HD-GLIO-AUTO-CoLlAGe"]:  
...
# Create columns for knowing how many past images each timepoint has (agreggated by groups of images)
...
for images in [["CT1"],["CT1","FLAIR"],["CT1","T1","T2","FLAIR"],["T1","T2","FLAIR"],["T1","FLAIR"]]:
...
for images_to_count in [["CT1", "T1", "T2", "FLAIR"], ["CT1", "FLAIR"],["T1", "T2", "FLAIR"],["T1", "FLAIR"],["CT1"]]:
    mods_to_count = "_".join(images_to_count)

    table_classifyable = remove_timepoints_rano(all) # remove less than 3 months and pre and post op
    ...
    for ind in table_classifyable.index[::-1]: #this index is the same as in table all
        count += 1
        path = os.path.join("./lumiere/datasets/rano_" + mods_to_count + "_T-1", table_classifyable["Patient"][ind] + "_" + table_classifyable["Timepoint"][ind])
        os.makedirs(path, exist_ok = True)

        for mod in images_to_count:
            # Add images of current timepoint
            image = os.path.join(data_dir, table_classifyable["Patient"][ind], table_classifyable["Timepoint"][ind], mod + "_tostd.nii.gz")
            ...
            image = os.path.join(data_dir, table_classifyable["Patient"][ind], all["Timepoint"][ind-1], mod + "_tostd.nii.gz") # Add image from one timepoint before
            if os.path.exists(image) and not os.path.exists(os.path.join(path, mod + "_T-1.nii.gz")):
