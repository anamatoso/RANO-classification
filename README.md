...
Afterwards, add the following transformation to the file `./venv/lib/python3.8/site-packages/monai/transforms/utility/dictionary.py` in `line 926` and add its name (`SubtractItemsd`) to `line 159`. Additionally, add its name also to the file `./venv/lib/python3.9/site-packages/monai/transforms/__init__.py` in `line 622` so that the package is aware of it.
