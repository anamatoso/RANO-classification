...
def plot_slices(images):
    ...
    """Plots a 3x3 grid with the overlay of images in images with 3 slices, one for each plane, at 1/3, 1/2 and 3/4 of the length of that plane.
...
def force_delete_file(file):
    """This function forces deletes a file with filepath "file" thus it does not prompt anything to the user and does not yield any error.
...
def calculate_isotropy(resolution):
    """This function calculates the isotropy of an image by calculating the mean of the ratios between each voxel dimension
...
def extract_firstvolume(filename, out_filename=None):
    ...
    Args:
        ...
        out_filename (filename, optional): File name of the output image file which is the first image but with just the first volume. If no name is given, then it outputs the image in the nibabel.nifti1.Nifti1Image format. Defaults to None.
...
def preprocess(image, output, use_FSL=False, pipeline=1):
    ...
    if pipeline == 1: # Pipeline python
        ...
        # Create mask to prevent intensity normalization in every iteration of the bias field correction
        mask = ants.threshold_image(reoriented_ants,low_thresh=1e-5,high_thresh=np.inf,inval=1,outval=0,binary=True)
...
def check_files_in_subdirectories(root_dir, target_files):
    """This function iterates through the folders in the root_dir directory to check if every folder has the file sin target_files
...
def critical_error(y_pred, y):
    """This function calculated the probability of making  crucial error. A crucial error is either when the function predicts disease development (0 or 1) but is is actually response (2 or 3), or when it predicts response development but it is actually disease development

    Args:
        y_pred (tensor or list): Class prediction of the model. It is either a list or a tensor with the numbers corresponding to the predicted class of the model
        y (tensor or list): Real value of classes. It is either a list or a tensor with the numbers corresponding to the real classes of the inputs.
